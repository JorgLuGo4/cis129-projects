...
# Ask the user for input (number of items ordered)
num_coffees = int(input("Number of coffees bought? "))
num_muffins = int(input("Number of muffins bought? "))
num_cakepieces = int(input("Number of croissants bought? "))
...
subtotal_cakepiece = num_croissants * croissant_price
subtotal_tea = num_teas * tea_price

# Calculate before tax
subtotal = subtotal_coffee + subtotal_muffin + subtotal_cakepiece + subtotal_tea
...
if num_croissants > 0:
    print(f"{num_cakepieces} Croissant{'s' if num_cakepieces > 1 else ''} at ${cakepiece_price} each: ${subtotal_cakepiece:.2f}")
if num_teas > 0:
